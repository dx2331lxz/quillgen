    ...
    def post(self, request):
        ...
        if location_type == 0:
            ...
            while models.Text.objects.filter(name=name, user=user).exists():
                name = f'{name}_copy'
            text = models.Text.objects.create(content=content, user=user, name=name, size=size,
            ...
            text = models.Text.objects.create(user=user, name=name, size=size, location=location,
                                              location_type=location_type)
            return Response({'msg': '上传成功', 'id': text.id}, status=status.HTTP_200_OK)
        else:
...
class Photo(APIView):
    ...
    def post(self, request):
        ...
        if photo.size > 1024 * 1024 * 5:
            return Response({'msg': '文件大小不能超过5M'}, status=status.HTTP_400_BAD_REQUEST)
